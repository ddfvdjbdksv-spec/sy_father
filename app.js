...
const StorageEngine = {
    ...
    // forcePull=true → يتجاهل علامة الترحيل ويسحب من السحابة مباشرة
...
const db = {
    ...
    async save(modifiedTable = null) {
        // ⚡ إصلاح أداء: modifiedTable ممكن تبقى الآن:
        //   - null/undefined  → حفظ كامل (كل الجداول) — السلوك الأصلي، بدون أي تغيير
        //   - '__settings__'  → لا تُعاد كتابة أي جدول في IndexedDB إطلاقاً، فقط
        //                        الإعدادات/الـ localStorage (اللي كانت بتُحفظ أصلاً
        //                        دايماً بعد الشرط التالي، بغض النظر عن الجدول)
        //   - string          → حفظ جدول واحد فقط (نفس السلوك السابق تماماً)
        //   - string[]        → حفظ أكثر من جدول واحد في نفس الاستدعاء
        if (modifiedTable === '__settings__') {
            // لا شيء يُحفظ في IndexedDB هنا — فقط الإعدادات أدناه
        } else if (Array.isArray(modifiedTable)) {
            for (const table of modifiedTable) {
...
function saveExamAttendance() {
    ...
    db.save('scores'); // ⚡ إصلاح أداء: حفظ جدول الدرجات فقط
        ...
        <tr class="fade-in">
            ...
            <td>
                <div style="display:flex; gap:5px;">
                    ...
                    <button class="btn" title="واتساب الطالب" style="padding:5px 10px; background:#facc15; color:#78350f;" onclick="sendStudentTrackingWhatsApp(${s.id})"><i class="fab fa-whatsapp"></i></button>
                    <button class="btn" title="واتساب ولي الأمر" style="padding:5px 10px; background:#25D366; color:white;" onclick="sendParentTrackingWhatsApp(${s.id})"><i class="fab fa-whatsapp"></i></button>
...
function handleAddReward() {
    ...
    db.save('rewards'); // ⚡ إصلاح أداء: حفظ جدول الجوائز فقط
    renderShop();
...
function redeemReward(rewardId) {
    ...
    if (student && student.points >= reward.cost) {
        ...
        db.save('students'); // ⚡ إصلاح أداء: حفظ جدول الطلاب فقط
...
function removeStudentFromPresentToday(studentId) {
    ...
    db.save('attendance'); // ⚡ إصلاح أداء: حفظ جدول الحضور فقط
...
function archiveAbsenceSession() {
    ...
    db.save(['attendance', 'absenceSessions']); // ⚡ إصلاح أداء: حفظ الجدولين المتأثرين فقط
...
function deleteAbsenceSession(id) {
    ...
    db.save('absenceSessions'); // ⚡ إصلاح أداء: حفظ جدول الجلسات المؤرشفة فقط
    showAbsenceArchive();
}
...
function manualResetDailyTreasury() {
    ...
    if (pass === '1234') {
        ...
        // ⚡ إصلاح أداء: dailyTreasuryArchives يُحفظ صراحةً بالسطر التالي مباشرة،
        // والباقي مجرد إعدادات — لا داعي لإعادة كتابة كل جداول التطبيق هنا
        db.save('__settings__');
...
function autoArchiveDailyTreasury() {
    ...
    if (!lastDateStr) {
        ...
        // ⚡ إصلاح أداء: db.dailyTreasuryLastArchiveDate ليس جدولاً في IndexedDB
        // (مجرد تاريخ)، ولا داعي لإعادة كتابة كل الجداول (بما فيها آلاف
        // الطلاب والحضور) في كل مرة تُستدعى فيها هذه الدالة كل دقيقة.
        db.save('__settings__');
        return;
    }
    ...
    // ⚡ إصلاح أداء: أي بيانات فعلية اتغيّرت (payments/expenses/dailyTreasuryArchives)
    // اتحفظت بالفعل مباشرة داخل _archiveDateTreasury أعلاه. هذا الاستدعاء كان
    // يُعيد كتابة كل جداول التطبيق (بما فيها آلاف سجلات الطلاب والحضور) كل
    // 60 ثانية طوال مدة فتح التطبيق، وهو السبب الرئيسي للتهنيج التدريجي.
    db.save('__settings__');
...
function endSessionAndMarkAbsent() {
    ...
    db.save('attendance'); // ⚡ إصلاح أداء: حفظ جدول الحضور فقط (addToQueue يحفظ waQueue بنفسه)
...
function printCertificate() {
    ...
    printWindow.document.write(`
        ...
        <head>
            ...
            <link rel="stylesheet" href="${new URL('vendor/fontawesome.min.css', location.href).href}">
            <style>
...
function removeFromQueue(id) {
    ...
    db.save('waQueue'); // ⚡ إصلاح أداء: حفظ جدول رسائل الواتساب فقط
    renderWAQueue();
}
...
function processAndSaveGrade(studentObj, examId, mark, maxMarksOverride) {
    ...
    db.save(['scores', 'students']); // ⚡ إصلاح أداء: حفظ الجدولين المتأثرين فقط بدل حفظ كامل + حفظ مكرر
...
function handleAddExam() {
    ...
    db.save('exams'); // ⚡ إصلاح أداء: حفظ جدول الامتحانات فقط
    renderExams();
...
function processScan(token) {
    ...
    // ⚡ إصلاح أداء: نحفظ فقط الجداول اللي فعلاً اتغيّرت في هذه الدالة
    // (حضور/نقاط الطالب/توزيع الملازم) بدل إعادة كتابة كل جداول التطبيق
    // في كل مرة يُسجَّل فيها حضور طالب — وهو ما كان يُبطئ الواجهة تدريجياً
    // كلما زاد عدد الطلاب وسجلات الحضور المتراكمة.
    db.save(['attendance', 'students', 'studentHandouts']);
...
function endLessonCoding() {
    ...
    // ── 4. حفظ البيانات ─────────────────────────────────────────
    // ⚡ إصلاح أداء: حفظ جدول الحضور فقط (waQueue/absenceSessions تُحفظ من دوالها الخاصة)
    db.save('attendance');
...
function startMonthlySubscription() {
    ...
    db.save('__settings__'); // ⚡ إصلاح أداء: تغييرات إعدادات فقط، لا داعي لحفظ كل الجداول
...
function promptEndMonthlySubscription() {
    ...
    if (pass === correct) {
        ...
        db.save('cycles'); // ⚡ إصلاح أداء: حفظ جدول الدورات فقط (والإعدادات تُحفظ دائماً تلقائياً)
...
function collectMonthlyPayment(studentId) {
    ...
    if (!db.settings.activeCycle) {
        if (confirm(`لا يوجد دورة اشتراك نشطة حالياً. هل تريد بدء دورة جديدة بقيمة ${db.settings.monthlyFee} ج.م؟`)) {
            ...
            db.settings.activeCycleGroupId = currentGroupId;
            db.settings.activeCycleGrade = currentGrade;
            db.save('__settings__'); // ⚡ إصلاح أداء: تغييرات إعدادات فقط
        } else {
            return;
        }
    }
...
// ============================================================
//  نظام رابط متابعة الطالب (Student Tracking Link)
// ============================================================
//  كل طالب أصلاً له كود فريد (s.qrCode) يولّده code-generator.js
//  عند التسجيل — نعيد استخدام نفس الكود كمُعرّف صفحة المتابعة بدل
//  توليد ID جديد، فلا يوجد داعٍ لأي تغيير في مكان آخر.
//
//  📌 ملاحظة مهمة: الرابط حالياً مبني على مسار عادي
//  (domain.com/student.html?id=CODE) وليس Subdomain مخصص
//  (student1.domain.com)، لأن الـ Subdomain يتطلب إعداد DNS/Wildcard
//  على مستوى الاستضافة (خارج نطاق كود التطبيق نفسه). لو الاستضافة
//  تدعم ذلك لاحقاً، تكفي إضافة إعادة توجيه Subdomain → نفس هذا المسار
//  مع تمرير الكود، ودالة getStudentTrackingLink أدناه هي المكان
//  الوحيد اللي يحتاج تعديل لتغيير شكل الرابط.
//
...
function sendParentTrackingWhatsApp(id) {
    const s = db.students.find(x => x.id === id);
    if (!s) return;
    ...
    const msg = buildFormalParentMessage({
        ...
        bodyLines: [
            `يمكنكم متابعة بيانات الطالب/ـة *${s.name}* (الحضور، الغياب، حالة الاشتراك، تقييم الأداء) في أي وقت من خلال الرابط التالي:`,
            link
        ]
    });
...
function recordArchivedMonthPayment(cycleId, studentId) {
    ...
    // ⚡ إصلاح أداء: سجل التعديلات جزء من _settings ويُحفظ دائماً تلقائياً
    db.save('payments');
...
function saveTreasuryArchiveHour(hour) {
    ...
    db.save('__settings__'); // ⚡ إصلاح أداء: تغيير إعدادات فقط
...
function runManualTreasuryArchiveNow() {
    ...
    localStorage.setItem('dt_last_archive_date', todayStr);
    localStorage.setItem('dailyTreasuryLastArchiveDate', todayStr);
    db.save('__settings__'); // ⚡ إصلاح أداء: باقي الجداول اتحفظت بالفعل داخل _archiveDateTreasury
...
const exposures = {
    ...
    getStudentTrackingLink, sendStudentTrackingWhatsApp, sendParentTrackingWhatsApp,
...
async function handlePunchPassword() {
    try {
        ...
        db.save('shifts'); // ⚡ إصلاح أداء: حفظ جدول الورديات فقط
        resultDiv.style.display = 'block';
        input.value = '';
...
// ============================================================
function updateFirebaseSyncStatusUI(state) {
    const dot = document.getElementById('firebase-sync-dot');
    const text = document.getElementById('firebase-sync-text');
    const lastTime = document.getElementById('firebase-sync-last-time');
    
    if (!dot || !text) return;
    
    const states = {
        checking: { color: '#f59e0b', msg: 'جاري التحقق من الاتصال...' },
        syncing: { color: '#3b82f6', msg: 'جاري المزامنة مع السحابة...' },
        latest: { color: '#10b981', msg: '✅ متصل ومزامن بالكامل' },
        offline: { color: '#94a3b8', msg: '⚫ غير متصل بالإنترنت' },
        error: { color: '#ef4444', msg: '❌ فشل الاتصال / خطأ قواعد الأمان' }
    };
    ...
    if (state === 'latest') {
        ...
        localStorage.setItem('edu_firebase_last_sync_time', timeStr);
        if (lastTime) lastTime.textContent = 'آخر مزامنة ناجحة: اليوم الساعة ' + timeStr;
    } else {
        const savedTime = localStorage.getItem('edu_firebase_last_sync_time');
        if (lastTime && savedTime) {
            lastTime.textContent = 'آخر مزامنة ناجحة: اليوم الساعة ' + savedTime;
        }
    }
}

async function triggerManualFirebaseSync() {
    const btn = document.getElementById('btn-firebase-sync');
    if (btn) {
        btn.disabled = true;
    ...
    try {
        if (!window.firebaseDB) {
            throw new Error("قاعدة بيانات Firebase غير متوفرة أو لم يتم تحميلها بعد.");
        }
        showNotification('🔍 جاري بدء المزامنة الثنائية مع Firebase...', 'info');
        await StorageEngine.syncWithFirebase(true); // force pull
        showNotification('✅ تمت المزامنة السحابية بنجاح!', 'success');
        updateFirebaseSyncStatusUI('latest');
    } catch (err) {
        console.error(err);
        showNotification(`❌ فشلت المزامنة: ${err.message || err}`, 'error');
        updateFirebaseSyncStatusUI('error');
    } finally {
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-sync-alt"></i> مزامنة البيانات سحابياً الآن ☁️';
        }
    }
}

function resetFirebaseMigrationFlag() {
    if (confirm("هل أنت متأكد من إعادة تهيئة المزامنة؟ هذا الخيار سيجعل التطبيق يرفع البيانات المحلية الحالية إلى Firebase مجدداً كعملية ترحيل أولية عند التحديث القادم.")) {
        localStorage.removeItem('edu_firebase_migrated');
        showNotification('🔄 تم إعادة تعيين مؤشر المزامنة. يرجى تحديث الصفحة للبدء.', 'info');
        setTimeout(() => window.location.reload(true), 1500);
    }
}

// Export functions to global scope
window.updateFirebaseSyncStatusUI = updateFirebaseSyncStatusUI;
window.triggerManualFirebaseSync = triggerManualFirebaseSync;
window.resetFirebaseMigrationFlag = resetFirebaseMigrationFlag;
